...
func GetContainer(env string) *Container {
	...
	aichatserviceClient := rpc.NewAIChatClient(configConfig)
	iaiChatController := aichat.NewAIChatController(aichatserviceClient)
	container := NewContainer(configConfig, traceSturct, logger, iUserController, iAuthController, iProjectController, iFeedbackController, iRoomController, iFileController, authserviceClient, client, fileserviceClient, aichatserviceClient, iaiChatController)
	return container
}
